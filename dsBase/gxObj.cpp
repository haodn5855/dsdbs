...
int CgxObj::AddRef()
{
return (int)InterlockedIncrement(&m_RefCount) ; 
}
int CgxObj::ReleaseRef()
{
	long Tmp = InterlockedDecrement(&m_RefCount) ;	
	if(! InterlockedCompareExchange(&m_RefCount , m_RefCount , 0) )
	{
		delete this ; 
	}
return (int)Tmp ;
